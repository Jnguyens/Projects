...
ALTER TABLE nba2
DROP COLUMN oreb_pct;

ALTER TABLE nba2
DROP COLUMN dreb_pct;
...
# Highest points/rebounds/assists per game average
...
FROM nba2
WHERE gp >= 55
ORDER BY reb DESC, pts, ast;

SELECT player_name, age, gp, pts, reb, ast, season 
FROM nba2
WHERE gp >= 55
ORDER BY ast DESC, pts, reb;

# Average points/rebounds/assists per game by every player from each season
...
SELECT season, AVG(reb)
FROM nba2
WHERE gp >= 55
GROUP BY season
ORDER BY 2 DESC;

SELECT season, AVG(ast)
FROM nba2
WHERE gp >= 55
GROUP BY season
ORDER BY 2 DESC;
